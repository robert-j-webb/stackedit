...
Eval is unsafe, but so is writing to the DOM. It’s true that it’s dangerous to write to the DOM unescaped, but very frequently we do that - for example allowing a user to have a link in their bio, or when a user uploads an image to a server and you render it. Markdown, which this blog post is written in, is compiled to HTML, sanitized, and then injected into the DOM.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYwMDkyNjkyMCwtODY4NjY2Mzg3LDM3Nz
